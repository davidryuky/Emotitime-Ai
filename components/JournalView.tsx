...
import { X, ChevronLeft, ChevronRight, Check, Edit3, Palette } from 'lucide-react';
...
const JournalView: React.FC<JournalViewProps> = ({ themeId, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-[#0a0a0a] z-[250] animate-in slide-in-from-right duration-500 overflow-y-auto flex flex-col font-satoshi">
        ...
        <div className="grid grid-cols-7 gap-2">
          ...
          {days.map(day => {
            ...
            return (
              ...
              >
                <span className={`text-base font-bold ${isTodayDate ? 'text-white' : 'text-gray-500'}`}>
                  {format(day, 'd')}
                </span>
      ...
      {selectedDate && (
        <div className="fixed inset-0 z-[300] bg-black/95 backdrop-blur-xl flex flex-col animate-in fade-in zoom-in-95 duration-300">
          <header className="p-6 pt-14 flex items-start justify-between border-b border-white/5">
            <div className="flex flex-col gap-3">
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-500">Escrevendo para</p>
